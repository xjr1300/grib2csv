...
3. ローカル・リポジトリの`main`ブランチに、リモート・リポジトリ（フェッチしたリポジトリ）の`main`ブランチをマージします。
4. `grib2csv`をコンパイルします。

```bash
# `grib2csv`ディレクトリをカレント・ディレクトリに
cd grib2csv
# リポジトリから最新のコードを取得（フェッチ）
git fetch
# `main`リポジトリをマージ
...
- 0.1.4
  - 2023-03-02 リリース
  - 気象庁が公開するサンプルデータを使用してテストを実行するように改修
